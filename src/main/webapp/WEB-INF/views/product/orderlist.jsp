   ...
   <div class="orderlist_section">
      ...
      <section>
         <c:forEach var="orderList" items="${orderLists}">
           <div class="order_num">
               <span>주문번호</span>	
               <span class="order_group_number">${orderList[0].order_group_number}</span>
               </div>
            <table class="orderlist_table" border="1" style="border-collapse: collapse;">
            	<c:forEach var="order" items="${orderList}">
	               <tr class="tr_order">
	                  <td class="td_product_img" >
	                     <img id="orderlist_product_img" src='<c:url value = "/product/img/${order.product_id}"/>'>
		               	  <input type="hidden" class="order_number" value="${order.order_number}">
	                  </td>
	                  <td>
	                     <span> ${order.order_date}</span>
	                     <div class="orderlist_product_name">${order.product_name}</div>
	                     <span class="orderlist_order_price">${order.order_price}원</span>
	                     <span class="orderlist_order_count">${order.order_product_count}개 구매</span>
	                  </td>
	                  ...
	                  <td class="td_status_and_btn">
	                        ...
	                        <input class="review_popup_btn" type="button" value="후기 쓰기">
