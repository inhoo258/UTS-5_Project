...
<body>
	...
	<div class="noice_section">
		...
		<div align="center">
			<form action="">
				<sec:authorize access="hasAnyRole('ROLE_MASTER','ROLE_SELLER','ROLE_CUSTOMER')">
					<div class="btn_write"><a href="<c:url value='/board/qna/form'/>">글 쓰기</a></div>
        	</sec:authorize>
					...
					<tbody>
						<c:forEach var="list" items="${qnaList}" varStatus="status">
							<tr class="tr_notice">
								<td>${list.qna_rn}</td>
								<td class="td_title">
								<a	href='<c:url value="/board/qna/${list.qna_rn}"/>'>${list.qna_title}</a>
								</td>
								<td>${list.qna_date}</td>
