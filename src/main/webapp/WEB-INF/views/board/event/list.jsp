                ...
                <div>
                    ...
                    <div id=contents_notis_table>
                        <table>
                            <tr>
                          	...
                          	<c:forEach var="list" items="${eventList}" varStatus="status">
		                  		<tr>
			                    	<th>${list.event_number}</th>
			                     	<th class="td_title">
		                       			<a href='<c:url value="/board/event/${list.event_rn}"/>' class="a_title">${list.event_title}</a>
		                     		</th>
		                     		<th>Master</th>
				                    <th>${list.event_date}</th>
				                    <th>${list.event_views}</th>
		                  		</tr>
		               		</c:forEach>
		               		<tr>
		               			<th colspan="5">
		               			<div>
								<c:if test="${event_pagingManager.nowPage ne 1}">
				                     <a class="a_paging" href="<c:url value='/board/event/list?event_page=1'/>">처음</a>
				                </c:if> 
				                <c:if test="${event_pagingManager.nowBlock gt 1 }">
				                     <a class="a_paging" href='<c:url value="/board/event/list?event_page=${event_pagingManager.startPage-1}"/>'>이전</a>
				                </c:if> 
				                <c:forEach var="i" begin="${event_pagingManager.startPage}" end="${event_pagingManager.endPage}">
				                      <a class="a_paging a_num" href='<c:url value="/board/event/list?event_page=${i}"/>' onclick="change_color_pagingBtn()">${i}</a>
				                </c:forEach> 
				                <c:if test="${event_pagingManager.nowBlock lt event_pagingManager.totalBlock}">
				                     <a class="a_paging" href="<c:url value='/board/event/list?event_page=${event_pagingManager.endPage+1}'/>">다음</a>
				                </c:if> 
				                <c:if test="${event_pagingManager.nowPage ne event_pagingManager.totalPage}">
				                     <a href="<c:url value='/board/event/list?event_page=${event_pagingManager.totalPage}'/>">끝</a>
				                </c:if>
				                </div>
				                </th>
		               		</tr>
                        ...
                        <sec:authorize access="hasAnyRole('ROLE_MASTER','ROLE_SELLER')">
			           		<div id=notice_form><button onclick="location.href='/project/board/event/form'">글쓰기</button></div>
			        	</sec:authorize>
                    </div>
                </div>
                <div>
                            ...
                            <c:forEach var="list" items="${faqList}" varStatus="status">
		               		...
		               		<tr>
								...
								<c:if test="${fre_PagingManager.nowPage ne 1}">
				                     <a class="a_paging" href="<c:url value='/board/event/list?fre_page=1&fre=fre'/>">처음</a>
				                </c:if> 
				                <c:if test="${fre_PagingManager.nowBlock gt 1 }">
				                     <a class="a_paging" href='<c:url value="/board/event/list?fre_page=${fre_PagingManager.startPage-1}&fre=fre"/>'>이전</a>
				                </c:if> 
				                <c:forEach var="i" begin="${fre_PagingManager.startPage}" end="${fre_PagingManager.endPage}">
				                      <a class="a_paging a_num" href='<c:url value="/board/event/list?fre_page=${i}&fre=fre"/>' onclick="change_color_pagingBtn()">${i}</a>
				                </c:forEach> 
				                <c:if test="${fre_PagingManager.nowBlock lt fre_PagingManager.totalBlock}">
				                     <a class="a_paging" href="<c:url value='/board/event/list?fre_page=${fre_PagingManager.endPage+1}&fre=fre'/>">다음</a>
				                </c:if> 
				                <c:if test="${fre_PagingManager.nowPage ne fre_PagingManager.totalPage}">
				                     <a class="a_paging end_paging " href="<c:url value='/board/event/list?fre_page=${fre_PagingManager.totalPage}&fre=fre'/>">끝</a>
				                </c:if>
				                </div>
				                </th>
		               		</tr>
